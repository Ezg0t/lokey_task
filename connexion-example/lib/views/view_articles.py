    ...
    if year:
        try:
            ...
            query = query.filter(db.extract(
                'year', Article.release_date) == year)
        except ValueError:
            return {"message": "Invalid year format"}, HTTPStatus.BAD_REQUEST
    ...
    return [
        {
            ...
            'release_date': article.release_date.isoformat() if article.release_date else None,
        }
